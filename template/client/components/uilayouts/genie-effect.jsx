...
function renderGenie(ctx, off, W, H, rawT, dir, dock, win) {
    ...
    <div className="flex h-full">
      <div
        ...
        {[
          ...
          <div
            key={i}
            className={`px-2 py-1.5 rounded-md text-[10px] flex items-center gap-2 ${i === 0 ? "text-white" : "text-white/55"}`}
            style={
              i === 0 ? { background: "rgba(255,83,106,.18)" } : undefined
            }>
            <span
...
// ─── Mac window wrapper ───────────────────────────────────────────────────────
const MacWindow = ({ app, winPos, onClose, domRef }) => {
...
function SnapshotStage({ onReady }) {
  const refs = useRef([]);
  useEffect(() => {
    let cancelled = false;
...
export default function GenieEffect() {
  ...
  const handleSnapshotsReady = useCallback((canvases) => {
  ...
  const getDockCenter = useCallback((idx) => {
    ...
    <div
      ...
      <div
        ...
        <span className="text-white/70 flex items-center text-[11px] gap-1 font-semibold tracking-tight">
          <img src="/macicon.png" alt="macicon" className="w-4 h-4" />
          {app?.label ?? "Finder"} &nbsp; File &nbsp; Edit &nbsp; View &nbsp;
          Window &nbsp; Help
        </span>
